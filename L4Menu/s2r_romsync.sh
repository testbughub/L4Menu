...
function romsync() {
  for type in ${types[@]}; do
    if grep -q "^$type$" $exclude ; then
      ...
      if whiptail \
      --title "ROM Sync" \
      --msgbox "This will sync ROM's from the server to RetroPie.\nContinue?" ; then
        if sudo mount -t cifs -o credentials=/home/pi/.smbcredentials,uid=1000,iocharset=utf8 //$SERVERIP/roms $RMOUNTPATH ; then
          rsync -tvurP --exclude={'*.state*','*.srm','*.mp4','*.jpg','*.png','*.jpeg'} --include={'*.nds','*.cso','*.hi','*.nv','*.000','*.rts','*.grp','*.xml','*.cfg','*.zip','*.wad','*.A52','*.gb','*.rtc','*.GBA','*.gba','*.gbc','*.smd','*.n64','*.z64','*.nes','*.sh','*.iso','*.ISO','*.cue','*.bin','*.BIN','*.m3u'} /mnt/roms/$type/ /home/pi/RetroPie/roms/$type/
        else
          whiptail --title "ROM Sync" --msgbox "Unable to mount the server"
