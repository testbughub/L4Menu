...
if sudo mount -t cifs -o credentials=/home/pi/.smbcredentials,uid=1000,iocharset=utf8 //$SERVERIP/roms $RMOUNTPATH ; then
  if whiptail --title "Sync > Save" --yesno "This will sync save states from RetroPie to the server.\nFor the moment it doesn't follow your custom exclusions list.\nContinue?" 10 40 3 ; then
    sudo rsync -tvurmP --include={'*.state*','*.srm','*.smc'} --exclude={'*.nds','*.cso','*.hi','*.nv','*.000','*.rts','*.grp','*.xml','*.cfg','*.zip','*.wad','*.A52','*.gb','*.rtc','*.GBA','*.gba','*.gbc','*.smd','*.n64','*.z64','*.nes','*.sh','*.iso','*.ISO','*.cue','*.bin','*.BIN','*.m3u','*.mp4','*.jpg','*.png','*.jpeg'} /home/pi/RetroPie/roms/* $RMOUNTPATH/
